...
export default function App() {
  ...
  // Helper utility to safely unwrap nested JSON and clean up escape sequences
  const cleanAndFormatJson = (inputData) => {
    if (!inputData) return '';
    try {
      // 1. Convert to a real object if it arrived as a raw string
      let parsedObj = typeof inputData === 'string' ? JSON.parse(inputData) : inputData;
      
      // 2. Loop through and clean up nested arguments strings if present
      if (Array.isArray(parsedObj)) {
        parsedObj = parsedObj.map(item => {
          if (item?.function?.arguments && typeof item.function.arguments === 'string') {
            try {
              item.function.arguments = JSON.parse(item.function.arguments);
            } catch (e) {
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <header className="mb-10 text-center md:text-left">
          ...
          <p className="text-slate-400 text-sm md:text-base">
            Fine-tuned Qwen 1.5B edge agent dynamically routing requests to official endpoints.
          </p>
        </header>
        ...
        {/* --- INTERACTIVE TERMINAL PREVIEW BLOCK --- */}
        {rawJson && (
          <section className="mt-8 bg-slate-950 border border-slate-800 rounded-xl overflow-hidden shadow-2xl animate-fadeIn">
            ...
            <div className="p-5 font-mono text-sm leading-relaxed overflow-x-auto text-emerald-400 bg-slate-950">
              <div className="text-slate-500 mb-2">// Exact structural JSON tool payload generated by fine-tuned model:</div>
              <pre className="whitespace-pre-wrap break-all text-blue-300">
                {cleanAndFormatJson(rawJson)}
              </pre>
            </div>
          </section>
        )}
